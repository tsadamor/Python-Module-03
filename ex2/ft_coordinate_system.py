...
def get_player_pos() -> tuple[float, float, float]:
    while True:
        ...
        try:
            current_param = x_str
            x = float(current_param)
            current_param = y_str
            y = float(current_param)
            current_param = z_str
            z = float(current_param)
            return (x, y, z)

        except ValueError:
            print(
                f"Error on parameter '{current_param}': "
                f"could not convert string to float: '{current_param}'"
                )
...
def main() -> None:
    ...
    print(f"It includes: "
          f"X={pos_1[0]}, Y={pos_1[1]}, Z={pos_1[2]}")
    print(f"Distance to center: "
          f"{calculate_distance(pos_1, (0.0, 0.0, 0.0))}\n")

    print("Get a second set of coordiantes")
    pos_2 = get_player_pos()
    print(f"Distance between the 2 sets of coordinates: "
          f"{calculate_distance(pos_1, pos_2)}")
